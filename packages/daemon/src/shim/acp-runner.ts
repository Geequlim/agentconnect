...
export class AcpRunner {
  ...
  /** Chunk writes in arrival order. The client serves requests concurrently, and the dedupe below is
   *  only a dedupe if the re-send reads `appliedSeq` AFTER the attempt it duplicates has recorded it. */
  ...
  async apply(rawPayload: unknown): Promise<void> {
    ...
    if (payload.op === 'chunk') {
      if (!this.child?.stdin) throw new Error('acp stream is not open')
      const run = this.writes.then(() => this.writeChunk(payload.data, payload.seq))
      // A failed write does not take the chain down with it; the retry that follows rides it.
  ...
  private async writeChunk(data: string, seq: number | undefined): Promise<void> {
    ...
    if (seq !== undefined && seq <= this.appliedSeq) return
    ...
    await new Promise<void>((resolve, reject) => {
      child.stdin!.write(Buffer.from(data, 'base64'), (err) => (err ? reject(err) : resolve()))
    })
    // Recorded AFTER the write, so a write that failed is still owed: the retry must apply.
    if (seq !== undefined) this.appliedSeq = seq
  }
